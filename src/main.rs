...
fn calculate_step_size(n: f64) -> (f64, usize) {
    let e = n.log10().floor();
...
fn simulate(l: f64, g: f64, i: f64, n: f64) {
    let (dt, print_interval) = calculate_step_size(n);
    eprintln!("dt = {}", dt);
...
fn peaks(i: f64, start: f64, end: f64, step: f64, g: f64, n: f64) {
    let (dt, _) = calculate_step_size(n);
    let initial_condition = arr1(&[n-i, i, 0.0]);
...
fn main() {
    ...
    match opt {
        ...
        Opt::Peaks { start, end, step, gamma, infected, population } => {
            eprintln!("# peaks");
            eprintln!("## parameters");
            eprintln!("- l : {}:{}", start, end);
            eprintln!("- g : {}", gamma);
            eprintln!("## initial values");
            eprintln!("- S : {}", population-infected);
            eprintln!("- I : {}", infected);
            eprintln!("- R : {}", 0.0);
            peaks(infected, start, end, step, gamma, population);
        }
    }
}
